...
import { scrapeMaajim, askAnotherGPT4 } from '$lib/server/scraping';
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    const data = await scrapeMaajim(input);
    return askAnotherGPT4(input, data.relatedSentences.join('\n'))
      .then((firstResult) => {
...
function replaceLetters(word1: string, word2: string, word3: string): string {
  ...
  const replacedWord = word1
    .split('')
    .map((letter) => lettersToReplace[letter] || letter)
    .join('');
  return replacedWord;
}
